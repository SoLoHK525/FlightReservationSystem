...
FROM FLIGHTS
WHERE SOURCE = 'HK' AND DEST = 'Tokyo'
  AND (DEPART_TIME - ARRIVE_TIME) * 24 < 24;


SELECT F1.FLIGHT_NO, F2.FLIGHT_NO, F1.FARE+F2.FARE
FROM FLIGHTS F1, FLIGHTS F2
WHERE (F1.SOURCE = 'HK' AND F2.DEST = 'New York')
  AND (F1.DEST = F2.SOURCE  AND F1.ARRIVE_TIME <= F2.DEPART_TIME)
  AND (F2.ARRIVE_TIME - F1.DEPART_TIME) * 24 < 24;
...
WHERE (F1.SOURCE = 'HK' AND F3.DEST = 'LA')
  ...
  AND(F3.ARRIVE_TIME - F1.DEPART_TIME) * 24 < 12;